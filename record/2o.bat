...
findstr /i /C:"Available streams" < "%scriptpath%\log\ck.txt" || (
	echo Streamlink can not use this link for live stream [ live.bat ]
...
%streamlink% "%streamurl%" %streamopt% --stdout | "%ffmpeg%" -i pipe:0 -acodec libmp3lame -ar 44100 -b:a 96k -pix_fmt yuv420p -profile:v baseline -bufsize 6000k -vb 400k -maxrate 1000k -deinterlace -vcodec libx264 -preset veryfast -g 30 -r 25 -crf 30 -f flv - | "%ffmpeg%" -f flv -i - -c copy -f flv "rtmp://live-api-s.facebook.com:80/rtmp/249230595853015?s_ps=1&s_vt=api-s&a=AThIWvRa8ZTc5RyR" -c copy -f flv %filename%
call :getTime now
if "%now%" leq "%endtime%" (
	goto :start_record
) else (
	if %rlog% equ 1 ( echo [ %time% ] - [ Stop record ] [ live.bat ] >> "%scriptpath%\log\%kenh%.log" )
