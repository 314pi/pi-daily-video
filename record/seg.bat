...
"%ffmpeg%" -fflags +genpts -i %filename% -map 0 -c copy -segment_times %del1%,%sub1%,%fil1%,%adv1%,%fil2%,%adv2%,%fil3%,%sub2% -f segment  -segment_format mp4 -reset_timestamps 1 -v error seg%%d.mp4
...
"%ffmpeg%" -v error -i seg1.mp4 -strict experimental %subtext1% -vcodec libx264 -crf 30 -c:a copy -ss 0 -t 10800 sub1.mp4
del seg1.mp4
ren sub1.mp4 seg1.mp4
if %special% equ 1 (
	"%ffmpeg%" -v error -i seg1.mp4 -i %logo% -strict experimental -vcodec libx264 -crf 30 -filter_complex "overlay=main_w-overlay_w-5:5" -c:a copy -ss 0 -t 10800 sub1.mp4
	del seg1.mp4
	ren sub1.mp4 seg1.mp4
)
...
echo Making SUB 2 ...
timeout /t 3 /nobreak >NUL
"%ffmpeg%" -v error -i seg7.mp4 -strict experimental %subtext2% -vcodec libx264 -crf 30 -c:a copy -ss 0 -t 10800 sub2.mp4
del seg7.mp4
...
"%ffmpeg%" -v error -i join_%name% -vcodec libx264 -crf 30 %speed% -ss 0 -t 10800 comp_%name%
:ig_compress
